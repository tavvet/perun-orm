...
/// ORM role of one mapped entity field.
public enum ColumnRole: Sendable, Hashable {
    /// A regular persisted attribute.
    case attribute
    /// The entity primary key, optionally generated by the database.
...
public struct FieldDescriptor<Root: Sendable> {
    /// The unquoted database column name.
    public let column: String
    /// The portable column type derived from the mapped Swift value.
    public let type: ColumnType
    /// Whether the mapped Swift value accepts SQL `NULL`.
    public let isNullable: Bool
    /// Whether generated DDL marks this column unique.
    public let isUnique: Bool
    /// The structural role of the mapped field.
    public let role: ColumnRole
    /// The entity key path represented by this descriptor.
    public let keyPath: PartialKeyPath<Root>

    let readValue: (Root) -> SQLValue
    let containsReferenceType: Bool

    /// Creates metadata for one persistable property.
    ...
    /// Reads the mapped property from `root` as a semantic DBAL value.
...
public protocol Entity: Sendable {
    /// The non-optional portable Swift type of the entity primary key.
    associatedtype PK: SQLValueConvertible & Hashable

    /// The unquoted table name used by generated statements.
    static var tableName: String { get }
    /// Complete mapping metadata, including exactly one primary-key descriptor.
    static var fields: [FieldDescriptor<Self>] { get }

    /// Hydrates an entity from a row containing every mapped column.
    init(row: any Row) throws
}

public extension Entity {
    /// Returns the declared descriptor for `keyPath`, if it is mapped.
    static func field(for keyPath: PartialKeyPath<Self>) -> FieldDescriptor<Self>? {
        fields.first { $0.keyPath == keyPath }
    }
}

/// Failures validating an entity's declared mapping before SQL execution.
public enum EntitySchemaError: Error, Sendable, Equatable {
    /// The entity itself is a reference type and cannot provide snapshot semantics.
    case referenceTypeUnsupported
    /// A directly mapped field contains reference semantics.
    case referenceFieldTypeUnsupported(column: String)
    /// The declared table name is empty.
    case emptyTableName
    /// The entity declares no mapped fields.
    case noFields
    /// A mapped field has an empty column name.
    case emptyColumn(index: Int)
    /// Two mapped fields use the same column name, ignoring case.
    case duplicateColumn(String)
    /// The same entity key path is declared more than once.
    case duplicateKeyPath(column: String)
    /// The mapping does not declare exactly one primary key.
    case primaryKeyCount(Int)
    /// The mapped primary key or `Entity.PK` type is optional.
    case nullablePrimaryKey(String)
    /// The primary-key descriptor type differs from `Entity.PK`.
    case primaryKeyTypeMismatch(expected: ColumnType, actual: ColumnType)
    /// The selected portable type is not stable as a primary key.
...
public struct EntitySchema<E: Entity> {
    ...
    /// The complete validated field list in declaration order.
    public let fields: [FieldDescriptor<E>]
    /// The single validated primary-key descriptor.
    public let primaryKey: FieldDescriptor<E>

    /// Validates and freezes the structural mapping for `entity`.
